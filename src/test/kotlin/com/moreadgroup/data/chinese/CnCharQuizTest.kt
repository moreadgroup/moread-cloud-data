...
//    private val ROOT_FOLDER = "/Users/CC/github/moreadgroup/moread-cloud-data/src/main/resources/"
    ...
    fun testGeneratePinyinQuizzesThenOK() {
        ...
        Files.newBufferedReader(Paths.get(srcFolder + "cnchar.csv")).use { reader ->
            ...
            val words = mutableListOf<CnCharLine>()
    ...
    private fun generatePinyinQuizItemsFromWords(
        wordLines: MutableList<CnCharLine>,
    ...
    private fun generatePinyinQuizItemLineFromWord(
        ...
        val usedWords = mutableListOf<CnCharLine>(cnWord)
        if ("A" != qil.ans) {
    ...
    private fun generateStrokeQuizItemLineFromWord(cnWord: CnCharLine): QuizItemLine {
    ...
    private fun shuffleStrokes(cnWord: CnCharLine): String? = Optional.ofNullable(cnWord.strokes).map {
        var result: String? = "X"
        val ars = it.split(" ")
        if (ars.size > 3) {
